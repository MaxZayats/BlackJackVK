...
token = 'some_group_longpoll_token'
...
def get_first_hand(user_id, peer_id):
    ...
    vk.messages.send(
        ...
        keyboard=create_keyboard('Раздать Карты'),
        random_id=0
    )
...
def more(user_id, peer_id):
    ...
    vk.messages.send(
        peer_id=peer_id,
        message=f'Вы получаете: {players[user_id].player_hand[-1]}',
...
def stand(user_id, peer_id):
    while True:
        ...
        vk.messages.send(
            peer_id=peer_id,
            message=f'Дилер получает: {players[user_id].dealer_hand[-1]}',
            keyboard=create_keyboard('Хватит'),
            random_id=0
        )
        ...
        if players[user_id].dealer_sum > 21:
            ...
            vk.messages.send(
                peer_id=peer_id,
                message='Дилер набрал больше 21 очка.\nВы выиграли!',
                keyboard=create_keyboard('Меню'),
                random_id=0
            )
            players[user_id].close_game()
    ...
    if players[user_id].dealer_sum < players[user_id].player_sum:
        vk.messages.send(
            peer_id=peer_id,
            message='Вы набрали больше чем дилер.\nВы выиграли!',
            keyboard=create_keyboard('Меню'),
            random_id=0
        )
        update_score(user_id, 1)
        ...
        vk.messages.send(
            peer_id=peer_id,
            message='Дилер набрал больше вас.\nВы проиграли!',
            keyboard=create_keyboard('Меню'),
            random_id=0
        )
        update_score(user_id, -1)
        ...
        vk.messages.send(
            peer_id=peer_id,
            message='Дилер набрал такое же количество очков, что и вы.\nНичья!',
            keyboard=create_keyboard('Меню'),
            random_id=0
        )
...
def double(user_id, peer_id):
    ...
    vk.messages.send(
        peer_id=peer_id,
        message=f'Вы получаете: {players[user_id].player_hand[-1]}',
        keyboard=create_keyboard('Удвоить Ставку'),
        random_id=0
    )
    vk.messages.send(
        peer_id=peer_id,
                ...
                if event.from_user:
                    if payload == '{"command":"start"}':
                        ...
                        if message == 'Меню':
                            ...
                            vk.messages.send(
                                peer_id=peer_id,
                                message='Бот воспринимает команды только с кнопок.\n'
                        ...
                        try:
                            if players[user_id].number_of_game > NUM_OF_GAMES_TO_SHFL:
                                ...
                                vk.messages.send(
                                    peer_id=peer_id,
                                    message='Колода была перетасована!',
                                    random_id=0
                                )
                        except:
                            players.update({user_id:game.player()})

                        if players[user_id].game_is_open is not True:
                            if message == 'Раздать Карты':
                                get_first_hand(user_id, peer_id)
                            else:
                                vk.messages.send(
                                    peer_id=peer_id,
                        ...
                        elif players[user_id].game_is_open is True:
                            if message == 'Ещё':
                        ...
                        if message == 'Список Лидеров':
                            vk.messages.send(
                                peer_id=peer_id,
                                message=f'Список лидеров:\n{get_leaders(user_id)}',
                                random_id=0
                            )
                        elif message == 'Правила':
                            vk.messages.send(
                                peer_id=peer_id,
                                message='Правила довольно просты:\n\n'
